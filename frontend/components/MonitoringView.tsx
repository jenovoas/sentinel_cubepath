...
const IframePanel = ({ title, icon, panelId, color, height = "180px", className = "" }: any) => {
  ...
  // Grafana endpoint real — asumiendo puerto 3000 o proxy inverso en /grafana
  const src = `/grafana/d-solo/sentinel-ring0-overview/sentinel-ring-0-overview?orgId=1&panelId=${panelId}&theme=dark&from=now-15m&to=now&refresh=5s&kiosk`;
...
export function MonitoringView() {
  const { connected, error, status } = useTelemetry();
  const [dataLoaded, setDataLoaded] = useState(false);

  // Simular detección de carga de dashboards reales
  useEffect(() => {
    if (connected && status) {
    ...
    <div className="flex flex-col space-y-8 h-full animate-in fade-in duration-700 pb-12 overflow-y-auto custom-scrollbar pr-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className={clsx(
          ...
          (!connected || !!error) ? "bg-rose-500/10 border-rose-500/20" : "bg-emerald-500/10 border-emerald-500/20"
        )}>
          <div className={clsx("w-2 h-2 rounded-full animate-pulse", (!connected || !!error) ? "bg-rose-500" : "bg-emerald-500")} />
          <span className={clsx("text-[10px] font-black tracking-widest uppercase", (!connected || !!error) ? "text-rose-400" : "text-emerald-400")}>
            {error ? error : connected ? "Sincronía Total S60" : "Buscando Kernel..."}
          </span>
        </div>
      </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            <IframePanel panelId={1} color="slate" />
            <IframePanel panelId={2} color="slate" />
            <IframePanel panelId={5} color="slate" className="md:col-span-2 xl:col-span-2" />
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <IframePanel panelId={3} color="sky" />
            <IframePanel panelId={4} color="sky" />
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <IframePanel panelId={7} color="emerald" height="180px" />
            <IframePanel panelId={8} color="emerald" height="180px" />
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
           ...
           <div className="flex flex-col gap-4">
               <IframePanel panelId={9} color="rose" height="180px" />
           ...
           <IframePanel panelId={6} color="amber" height="100%" className="min-h-[260px] h-full" />
        </div>
      </section>
